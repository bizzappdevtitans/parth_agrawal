...
class FreightOrderServiceLine(models.Model):
    """This model will create service line that will be used in freight order model"""
    ...
    partner_id = fields.Many2one(
        "res.partner",
        related="service_id.partner_info_id",
        string="Vendor",
        readonly=False,
    )
    ...
    def _onchange_partner_id(self):
        ...
        for freight_service in self:
            if (
                freight_service.service_id
                and freight_service.partner_id
                and freight_service.service_id.line_ids
            ):
